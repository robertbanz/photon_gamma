...
int vInit(char adapter) {
  ...
  if (adapter == FINDADAPTER) {
    VideoID(&VIDstruct);
    adapter = VIDstruct.Video0Type;
  }
  if (adapter == MDA || adapter & 0x80) /* if MDA or Herc */
  {
    vbase.swidth = 80;
    vbase.height = 24;
    vbase.vrealmem = (unsigned short _far *)0xb0000000;
    vbase.CRTC = 0x3b4;
    vbase.cardref = 0;
    vbase.textmode = 7;
  } else {
    vbase.swidth = 40;
    vbase.height = 24;
    vbase.vrealmem = (unsigned short _far *)0xb8000000;
    vbase.CRTC = 0x3d4;
    vbase.cardref = adapter - 1;
    vbase.textmode = 1;
  }
  vbase.numidx = vbase.swidth * vbase.height;
  /*vmode(vbase.textmode);*/
  apages[0].pagemem = vbase.vrealmem;
  numbytes += vbase.swidth * 2; /* enough for a line of scrap */
  numbytes += vbase.numidx * 2; /* another screenfull */
  if (!(vbase.imagemem = (unsigned short _far *)_fmalloc(numbytes)))
    return (VNOMEM);
  _fmemset(vbase.imagemem, 0, numbytes);

  apages[1].pagemem = &vbase.imagemem[IM_MEMTOTAL / 2];
  vbase.linemem = &vbase.imagemem[IM_MEMTOTAL / 2 + vbase.numidx];
  vbase.curpage = 0;
...
void vBlinkControl(char onoff) {
  ...
  if (vbase.cardref < 2) {
    ...
    _asm mov bx,
        blink /* ax = onoff */
        _asm mov ax,
        0x1003 _asm int 0x10
  }
}

void vScrollUp(void) {
  unsigned short lp;
  lp = vbase.numidx - vbase.swidth;
  _fmemmove(vbase.pages.pagemem, &vbase.pages.pagemem[vbase.swidth],
            (vbase.numidx - vbase.swidth) * 2);
  for (; lp < vbase.numidx; lp++)
...
void vClearScreen(void) {
  unsigned short vidx = vbase.numidx;
...
void vPosCur(unsigned char nx, unsigned char ny) {
  if (nx >= vbase.swidth) nx = vbase.swidth - 1;
  if (ny >= vbase.height) ny = vbase.height - 1;
  vbase.pages.writepos.crsx = nx;
  vbase.pages.writepos.crsy = ny;
  vbase.pages.writepos.idx = (ny * vbase.swidth) + nx;
  vCupDat();
}
...
void vLF(void) {
  if (++vbase.pages.writepos.crsy >= vbase.height) {
    vScrollUp();
    vbase.pages.writepos.crsy--;
  } else {
    vbase.pages.writepos.idx += vbase.swidth;
    vCupDat();
  }
}
...
void vBACKS(void) {
  if (vbase.pages.writepos.idx) {
    ...
    if (vbase.pages.writepos.crsx)
      ...
      vbase.pages.writepos.crsx = (unsigned char)(vbase.swidth - 1);
      vbase.pages.writepos.crsy--;
    }
  }
  vCupDat();
...
void vHorizRel(unsigned char vector) {
  if (vector & HIGHBIT) {
    ...
    if (vbase.pages.writepos.crsx + vector < vbase.swidth) {
      vbase.pages.writepos.crsx += vector;
      vbase.pages.writepos.idx += vector;
    } else {
      vbase.pages.writepos.idx += vbase.swidth - vbase.pages.writepos.crsx - 1;
      vbase.pages.writepos.crsx = (unsigned char)(vbase.swidth - 1);
    }
  }
  vCupDat();
...
void vVertRel(unsigned char vector) {
  if (vector & HIGHBIT) {
    ...
    if (vbase.pages.writepos.crsy > vector) {
      ...
      vbase.pages.writepos.idx -= vector * vbase.swidth;
    } else {
      vbase.pages.writepos.idx = vbase.pages.writepos.crsx;
      vbase.pages.writepos.crsy = 0;
    }
  } else {
    if (vbase.pages.writepos.crsy + vector < vbase.height) {
      vbase.pages.writepos.crsy += vector;
      vbase.pages.writepos.idx += vector * vbase.swidth;
    } else {
      vbase.pages.writepos.idx =
          vbase.numidx - (vbase.swidth - vbase.pages.writepos.crsx);
      vbase.pages.writepos.crsy = (unsigned char)(vbase.height - 1);
    }
  }
  vCupDat();
}
...
void vRepeatHVC(unsigned char rchar, unsigned char vector) {
  ...
  if (vector & HIGHBIT) {
    ...
    if (vbase.pages.writepos.crsx + vector < vbase.swidth)
      ...
      xgoto = (unsigned char)vbase.swidth;
      cursoradjust = 1;
    }
...
void vCurLineToScrap(void) {
  ...
  _fmemcpy(vbase.linemem, &vbase.pages.pagemem[lindx], vbase.swidth * 2);
}

void vScrapToCurLine(void) {
  unsigned short lindx;
  lindx = vbase.pages.writepos.idx - vbase.pages.writepos.crsx;
  _fmemcpy(&vbase.pages.pagemem[lindx], vbase.linemem, vbase.swidth * 2);
}
...
void vDelCurLine(void) {
  ...
  _fmemmove(&vbase.pages.pagemem[lindx],
            &vbase.pages.pagemem[lindx + vbase.swidth],
            (vbase.numidx - lindx - vbase.swidth) * 2);
  lindx = vbase.numidx - vbase.swidth;
  for (; lindx < vbase.numidx; lindx++)
...
void vInsertCurLine(void) {
  unsigned short lindx, xat;
  lindx = vbase.pages.writepos.idx - vbase.pages.writepos.crsx;
  if (vbase.pages.writepos.crsy < (unsigned char)(vbase.height - 1)) {
    _fmemmove(&vbase.pages.pagemem[lindx + vbase.swidth],
...
void vInsertBlank(void) {
  ...
  lastinline =
      vbase.pages.writepos.idx - vbase.pages.writepos.crsx + vbase.swidth - 1;
  for (xat = vbase.swidth - 1; xat > vbase.pages.writepos.crsx; xat--) {
...
void vDeleteCur(void) {
  ...
  for (xat = vbase.pages.writepos.crsx + 1; xat < vbase.swidth; xat++) {
    vbase.pages.pagemem[lastinline] = vbase.pages.pagemem[lastinline + 1];
    lastinline++;
  }
}
...
void vBorder(unsigned char xsize, unsigned char ysize, unsigned char bstyle) {
  ...
  if (xsize + vbase.pages.writepos.crsx > vbase.swidth) /* trim width */
    xsize = (unsigned char)(vbase.swidth - vbase.pages.writepos.crsx);
  if (ysize + vbase.pages.writepos.crsy > vbase.height) /* trim height */
    ysize = (unsigned char)(vbase.height - vbase.pages.writepos.crsy);
...
void vRepeatVVC(unsigned char vector, unsigned char rchar) {
  ...
  if (vector & HIGHBIT) {
    ...
    if (vbase.pages.writepos.crsy + vector < vbase.height)
      ...
      ygoto = (unsigned char)vbase.height;
      cursoradjust = 1;
    }
    ...
    vbase.pages.writepos.idx -= (cursoradjust * vbase.swidth);
...
void vStatLine(char *message, char position, unsigned char color, char cleol) {
  ...
  addidx = vbase.numidx + position;
  checkidx = vbase.numidx + vbase.swidth;
  while (*message && addidx < checkidx)
    vbase.vrealmem[addidx++] = (color << 8) | *message++;
  if (cleol)
    while (addidx < checkidx) vbase.vrealmem[addidx++] = (color << 8);
}

void vClearEOL(void) {
  unsigned short aidx, acheck;
  aidx = vbase.pages.writepos.idx - vbase.pages.writepos.crsx;
  acheck = aidx + vbase.swidth;
...
void vCurType(unsigned char newtype) /* not to be called directly */
{
  ...
  if (!vbase.curpage) {
    outp(vbase.CRTC, 0xa);
    outp(vbase.CRTC + 1, cur_sizes[newtype][vbase.cardref][0]);
    outp(vbase.CRTC, 0xb);
    outp(vbase.CRTC + 1, cur_sizes[newtype][vbase.cardref][1]);
  }
}
...
void vAttrChange(unsigned char color, unsigned char runlength) {
  ...
  if (vbase.pages.writepos.crsx + runlength >= vbase.swidth)
    idxgoto =
        (vbase.swidth - vbase.pages.writepos.crsx) + vbase.pages.writepos.idx;
  else
...
void vLineScroll(unsigned char vector) {
  ...
  if (vector & HIGHBIT) {
    ...
    if (vector)
      ...
      return;
    if (vector + vbase.pages.writepos.crsx >= vbase.swidth)
      vector = vbase.swidth - vbase.pages.writepos.crsx;
...
void vWidthHeight(unsigned char *swidth, unsigned char *height) {
  *swidth = (unsigned char)vbase.swidth;
  *height = (unsigned char)vbase.height;
}
